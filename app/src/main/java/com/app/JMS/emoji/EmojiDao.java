...
public class EmojiDao {
    ...
    public List<EmojiBean> getEmojiBean(){
        ...
        while (cursor.moveToNext()){
            ...
            LogUtil.d("emounicode " + bean.getUnicodeInt());
        }
    ...
    public static String CopySqliteFileFromRawToDatabases(String SqliteFileName) throws IOException {
        ...
        //复制的话这里需要换成自己项目的包名
        File dir = new File("/data/data/"  + "com.app.gaolonglong.fragmenttabhost" + "/databases");

        if (!dir.exists() || !dir.isDirectory()) {
            dir.mkdir();
        }
        LogUtil.d("path " + dir.getAbsolutePath());

        File file= new File(dir, SqliteFileName);
        LogUtil.d("File Path " + file.getAbsolutePath() +"  and  " + file.getPath());
//        LogUtil.d("my path " + SplashActivity.getCacheDir().getPath() + " ");
        ...
        if (!file.exists()) {
            try {
                file.createNewFile();
                inputStream = MainApplication.mApplication.getClass().getClassLoader().getResourceAsStream("assets/" + SqliteFileName);
